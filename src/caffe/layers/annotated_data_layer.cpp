    ...
    if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
      ...
      if (num_bboxes == 0) {
        ...
        for (int item_id = 0; item_id < batch_size; ++item_id) {
          ...
          for (int g = 0; g < anno_vec.size(); ++g) {	//anno_vec.size() == num_classes.
            const AnnotationGroup& anno_group = anno_vec[g];
            for (int a = 0; a < anno_group.annotation_size(); ++a) {
              const Annotation& anno = anno_group.annotation(a);
              const NormalizedBBox& bbox = anno.bbox();
              top_label[idx++] = item_id;	//which one among batchsize.
              top_label[idx++] = anno_group.group_label();	//label.
              top_label[idx++] = anno.instance_id();	//for one image. which instance.
